...
bool is_whitespace(char c) { return ((c == ' ') || (c == '\t') || (c == -1)); }

int safe_string_access(string s, int pos) {
  if ((pos >= 0) && (pos < (int)s.length())) return s[pos];
  return -1;
}

// Finds a word (i.e. word=="foo" would match on "foo bar" or "a foo bar" but
// not on "foobar")
size_t find_word(string s, const char *word) {
  int p = s.find(word);
  return is_whitespace(safe_string_access(s, p - 1)) &&
                 is_whitespace(safe_string_access(s, p + strlen(word)))
             ? p
             : string::npos;
}
...
void check_for_const(string s) {
  ...
  if ((find_word(s, "const") != string::npos) &&
      (s.find("extern") == string::npos) && (blockDepth == 0)) {
    ...
    if (i < s.length()) {
      if ((s.rfind("const") > s.find("*")) || (s.find("*") == string::npos)) {
        ...
        j = i - 1;
        if ((s[i] == '=') &&
            (s[j] == ' '))  // Alek : to avoid breaking with name =
        {
          j--;
          i--;
        }
...
int main(int argc, char **argv) {
      ...
      if ((ch == 10) || (ch == 13)) {
        if (oneLine.length()) {
          if (oneLine.find(".ramsection \"ram.data\"") != string::npos) {
            ...
            currSection = 2;
            if (sectName.length()) {
              asmSections[currSection].push_back(".section \"." + sectName +
                                                 "\" superfree");
            } else
              asmSections[currSection].push_back(oneLine);
  ...
  for (i = 0; i < constVars.size(); i++) {
    ...
    while (j < asmSections[0].size()) {
      if (asmSections[0][j].find(constVars[i] + ' ') !=
          string::npos)  // +' ' to avoid var with same beginning name
      {
        break;
      } else {
