...
enum { MaxUnrollThreshold = 2000, LoopLossTolerance = 30000 };
...
int GenerateBRR(const s16 *cdata, u8 *output, int srclength, int srcloop,
                int srclooplen, int ffixed, int loopfixed, double amp);
...
static double ResampleLoop(s16 **Data, int &length, int &loopstart,
                           int &looplength, int amount) {
...
static void PadData(s16 **Data, int &length, int &loopstart, int &looplength,
                    int begin, int end) {
  // oof
  s16 *newdata = new s16[length + begin + end];

  s16 *out = newdata;
  for (int i = 0; i < begin; i++) *out++ = 0;
  for (int i = 0; i < length; i++) *out++ = (*Data)[i];
  for (int i = 0; i < end; i++) *out++ = 0;
...
void UnrollLoop(s16 **Data, int &length, int &loopstart, int &looplength,
                int unroll) {
...
Source::Source(const ITLoader::SampleData &source) {
  ...
  if (looplength) {
    if (looplength & 0xF) {
      ...
      if ((looplength * (1 + unroll)) < MaxUnrollThreshold) {
        ...
        tuning_factor = ResampleLoop(&cdata, length, loopstart, looplength,
                                     16 - (looplength & 15));
      }
    }
    if (loopstart & 15) {
  ...
  while (c_redo) {
    //			if( cinput )
    //				c_redo=GenerateBRR( cdata, Data, length, loopstart, looplength,
    //cinput->force_filter == -1 ? 4 : cinput->force_filter,
    //cinput->force_loop_filter == -1 ? 4 : cinput->force_loop_filter, amp );
    //			else
...
int GenerateBRR(const s16 *cdata, u8 *output, int srclength, int srcloop,
  ...
  while (redo_loopf) {
    ...
    for (; a < srclength; a += 16) {
      ...
      for (b = 0; b < 8; b++)
        output[w + 1 + b] = cbuffer[b * 2] * 16 +
                            cbuffer[b * 2 + 1];  // write data (big endian)
    ...
    if (srclooplen) output[w] = output[w] | SAMPHEAD_LOOP;
...
void BRR_CompressBlock(int *source, int *dest, cresult *presult, int ffixed) {
  ...
  for (filter = fmin; filter <= fmax; filter++) {
    ...
    for (r_shift = 12; r_shift >= 0; r_shift--) {
      ...
      if (block_error < block_errorb) {
        ...
        for (x = 0; x < 16; x++) block_datab[x] = block_data[x];
        for (x = 0; x < 16; x++) block_sampb[x + 2] = block_samp[x];
      }
    }
  }
  ...
  for (x = 0; x < 16; x++) dest[x] = block_datab[x];

  // copy affected sample data
  for (x = 0; x < 16; x++) presult->samp[x] = block_sampb[x + 2];
