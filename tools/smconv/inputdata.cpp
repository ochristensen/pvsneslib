...
enum { ARG_OPTION, ARG_INPUT, ARG_INVALID };
...
static bool strmatch(const char *source, const char *test) {
  for (int i = 0; test[i]; i++)
    if (source[i] != test[i]) return false;
  return true;
}
...
#define TESTARG2(str1, str2) \
  (strmatch(argv[arg], str1) || strmatch(argv[arg], str2))
                ...
                for( const TiXmlNode *child = doc->FirstChild();
                        ...
                        if( child->Type() == TiXmlNode::ELEMENT ) {
                                if( child->ValueTStr() == "soundbank" ) {
                                        ...
                                        targets.push_back( new SoundbankData(
   child->ToElement() ) );
                                }
                        }
                }
...
ModuleData(child->ToElement()) ); } else if( child->ValueTStr() == "effect" ) {
                                effects.push_back( new
EffectData(child->ToElement()) ); } else { printf( "Unknown soundbank member...
...
ModuleData::ModuleData( const TiXmlElement *source ) {
        ...
        for( const TiXmlAttribute *a = source->FirstAttribute();
                ...
                if( name == "file" ) filename = PATH + a->Value();
                        ...
                        if( !f ) {
                                printf( "Encountered unknown module
attribute.\n" );
                        }
                }
        }
        ...
        for( const TiXmlNode *child = source->FirstChild();
                ...
                if( child->Type() == TiXmlNode::ELEMENT ) {
                        ...
                        if( child->ValueTStr() == "sample" ) {
                                samples.push_back( new SampleData(
child->ToElement() ) );
                        }
                }
...
SampleData::SampleData( const TiXmlElement *source ) {
        ...
        for( const TiXmlAttribute *a = source->FirstAttribute();
                ...
                else if( name == "loopfilter" ) force_loop_filter =
a->IntValue(); else { printf( "Encountered unknown sample attribute.\n" );
                }
        }
