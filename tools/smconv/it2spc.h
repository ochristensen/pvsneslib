...
class Source {
 ...
 public:
  Source(const ITLoader::SampleData &);
  ~Source();
  ...
  int GetDataLength() const { return Length; }

  int GetLoopPoint() const { return Loop; }

  double GetTuningFactor() const { return TuningFactor; }
};
...
class Pattern {
 ...
 public:
  ...
  int GetExportSize() { return 1 + Data.size(); }
...
class Module {
 ...
 public:
  Module(const ITLoader::Module &, const std::vector<u16> &,
         const std::vector<u8> &, const std::vector<Source *> &);
  ...
  int GetExportSize_Header() const { return 616; }
