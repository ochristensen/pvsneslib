...
static void deletePtrVector(std::vector<T *> &vecs) {
  for (typename std::vector<T *>::iterator iter = vecs.begin(),
                                           ending = vecs.end();
       iter != ending; iter++) {
    if (*iter) delete (*iter);
  }
...
Module::Module(const char *filename) {
  ...
  for (int i = 0; i < 26; i++) Title[i] = file.Read8();
  ...
  for (int i = 0; i < 64; i++) ChannelPan[i] = file.Read8();

  for (int i = 0; i < 64; i++) ChannelVolume[i] = file.Read8();
  ...
  for (int i = 0; i < InstrumentCount; i++) InstrTable[i] = file.Read32();
  for (int i = 0; i < SampleCount; i++) SampleTable[i] = file.Read32();
  for (int i = 0; i < PatternCount; i++) PatternTable[i] = file.Read32();
  ...
  if (MessageLength) {
    ...
    for (int i = 0; i < MessageLength; i++) Message[i] = file.Read8();
  } else {
    Message = 0;
  }
...
Module::~Module() {
  ...
  delete[] Instruments;
  for (int i = 0; i < SampleCount; i++) delete Samples[i];
  delete[] Samples;
  for (int i = 0; i < PatternCount; i++) delete Patterns[i];
  delete[] Patterns;
  if (Message) {
    delete[] Message;
  }
...
Instrument::Instrument(IO::File &file) {
  file.Skip(4);  // IMPI
  DOSFilename[12] = 0;
  for (int i = 0; i < 12; i++) DOSFilename[i] = file.Read8();
  file.Skip(1);  // 00h
  ...
  Name[26] = 0;
  for (int i = 0; i < 26; i++) Name[i] = file.Read8();
  ...
  while (1) {
    ...
    switch (chunk_code) {
      ...
      case 0x20746D66:  // format chunk
        ...
        if ((chunk_size - 0x10) > 0) file.Skip((chunk_size - 0x10));
        ...
        //---------------------------------------------------------------------
        {
          ...
          for (t = 0; t < Length; t++) {
            ...
            for (c = 0; c < num_channels; c++) {
              dat += bit_depth == 8 ? ((int)file.Read8()) - 128
                                    : ((short)file.Read16());
            }
            dat /= num_channels;
