...
{
/* 0*/	"Japan ", "USA & Canada ", "Europe, Oceania & Asia ", "Sweden ",
/* 4*/	"Finland ", "Denmark ", "France ", "Holland ",
/* 8*/	"Spain ", "Germany, Austria & Switzerland ", "Italy ", "Hong Kong & China ",
/* c*/	"Indonesia ", "South Korea ", UNK, UNK,
};
...
int show_header(char * filename, FILE *fp) 
{
	...
	if (!valid_check) {
		...
		if ((snesheader.checksum + snesheader.checksum_c) == 0xffff && (snesheader.checksum != 0) && (snesheader.checksum_c != 0))
			valid_check = 1;
		if (!valid_check) {
			printf("\nERROR: No valid Header (hiRom or loRom) for file [%s].",filename);
			return 0;
		}
	}
	...
	// RAM
	if (((snesheader.cardtype & 0xf) == 1) || ((snesheader.cardtype & 0xf) == 2) ||
...
int change_checksum(char * filename, FILE *fp) 
{
	...
	if (quietmode == 0)
		printf("\nChange Checksum to %04Xh and Checksum complement %04Xh..",sum_crc,sum_crc^0xffff);
