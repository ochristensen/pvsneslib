...
static const int memory_amount[] = {
    ...
    /*$07 => $20000 bytes == 128 kilobytes, amount of RAM in Dezaemon - Kaite
       Tsukutte Asoberu */
    128,
...
typedef struct {
  char extended[16];        // extra information for header
                    ...
                    */
  /*FFC0*/ char title[21];  // Name of the ROM, typically in ASCII, using spaces
                            // to pad the name to 21 bytes.
  /*FFD5*/ unsigned char romlayout;  // ROM layout, typically $20 for LoROM, or
                                     // $21 for HiROM. Add $10 for FastROM.
  /*FFD6*/ unsigned char cardtype;   // Cartridge type, typically $00 for ROM
                                     // only, or $02 for ROM with save-RAM.
  /*FFD7*/ unsigned char ROMsize;    // ROM size byte.
  /*FFD8*/ unsigned char SRAMsize;   // RAM size byte.
  /*FFD9*/ unsigned char countrycode;  // Country code, which selects the video
                                       // in the emulator. Values $00, $01, $0d
                                       // use NTSC. Values in range $02..$0c use
                                       // PAL. Other values are invalid.
  /*FFDA*/ unsigned char license;  // Licensee code. If this value is $33, then
                                   // the ROM has an extended header with ID at
                                   // $ffb2..$ffb5.
  /*FFDB*/ unsigned char version;  // Version number, typically $00.
  /*FFDC*/ unsigned short checksum_c;  // Checksum complement, which is the
                                       // bitwise-xor of the checksum and $ffff.
  /*FFDE*/ unsigned short
      checksum;         // SNES checksum, an unsigned 16-bit checksum of bytes.
...
int find_addon_chip(snes_header *head) {
  ...
  // http://snesemu.black-ship.net/misc/-from%20nsrt.edgeemu.com-chipinfo.htm
  switch (head->cardtype) {
    ...
    case 0x05:
      ...
      /* DSP-3 is hard to detect. We exploit the fact that the only game has
         been manufactured by Bandai */
    ...
    case 0x1a:  // GSU-1 (21 MHz at start)
      if (head->romlayout == 0x20) supported_type = HAS_SUPERFX;
      break;
    ...
    case 0x34:
    case 0x35:
      if (head->romlayout == 0x23) supported_type = HAS_SA1;
      break;

    case 0x43:
    case 0x45:
      if (head->romlayout == 0x32) supported_type = HAS_SDD1;
      break;

    case 0x55:
      if (head->romlayout == 0x35) supported_type = HAS_RTC;
      break;
...
they only differ for the 'size' at 0x00ffd7 */
    ...
    case 0xf9:
      if (head->romlayout == 0x3a) supported_type = HAS_SPC7110_RTC;
      break;
...
int show_header(char *filename, FILE *fp) {
  ...
  int addr, i, company, has_ram = 0, has_sram = 0, valid_check = 0,
                        add_on_chip = 0;
  ...
  if (fread(&snesheader, sizeof(snes_header), 1, fp) < 1) {
    printf("\nERROR: Header (loRom) of file [%s] is not correct.", filename);
    return 0;
  }

  // a valid checksum is the biggest indicator of a valid header to check lo or
  // Hi rom.
  if ((snesheader.checksum + snesheader.checksum_c) == 0xffff &&
      (snesheader.checksum != 0) && (snesheader.checksum_c != 0))
    valid_check = 1;
  if (!valid_check) {
    ...
    // read the header
    if (fread(&snesheader, sizeof(snes_header), 1, fp) < 1) {
      printf("\nERROR: Header (hiRom) of file [%s] is not correct.", filename);
      return 0;
    }
    if ((snesheader.checksum + snesheader.checksum_c) == 0xffff &&
        (snesheader.checksum != 0) && (snesheader.checksum_c != 0))
      valid_check = 1;
    if (!valid_check) {
      printf("\nERROR: No valid Header (hiRom or loRom) for file [%s].",
             filename);
      return 0;
    }
  }
  ...
  // Company
  for (i = 0; i < 2; i++) company_id[i] = snesheader.extended[0 + i];
  company =
      (char_to_int_conv(company_id[0]) << 4) + char_to_int_conv(company_id[1]);
  if (company == 0) company = snesheader.license;

  // ROM ID
  for (i = 0; i < 4; i++) rom_id[i] = snesheader.extended[2 + i];
  rom_id[4] = '\0';

  /* Title */
  for (i = 0; i < 21; i++) title[i] = snesheader.title[0 + i];
  title[21] = '\0';

  // RAM
  if (((snesheader.cardtype & 0xf) == 1) ||
      ((snesheader.cardtype & 0xf) == 2) ||
      ((snesheader.cardtype & 0xf) == 4) || ((snesheader.cardtype & 0xf) == 5))
    has_ram = 1;

  // SRAM
  if (((snesheader.cardtype & 0xf) == 2) ||
      ((snesheader.cardtype & 0xf) == 5) || ((snesheader.cardtype & 0xf) == 6))
    has_sram = 1;
  ...
  if (showheader) {
    ...
    printf("\nGame title    : %s", title);
    printf("\nROM ID        : %.4s",
           snesheader.license == 0x33 ? rom_id : "None");
    printf("\nCompany       : %s [%02X]", companies[company], company_id[0]);
    printf("\nLicense       : %s [%02X]", companies[snesheader.license],
           snesheader.license);
    printf("\nCountry:      : %s [%02X]", countries[snesheader.countrycode],
           snesheader.countrycode);
    printf("\nVersion       : 1.%d", snesheader.version);
    if (snesheader.license == 0x33)
      printf("\nPromo Version : %d", snesheader.extended[14]);
    else
      printf("\nPromo Version : None");
    printf("\nHeader Type   : %s", snesheader.license == 0x33
                                       ? "Newer Extended Header"
                                       : "Old Header (not extended)");
    ...
    printf("\n--------------------");
    printf("\nROM Size      : %dK [%dK file]",
           memory_amount[snesheader.ROMsize], rom_size / 1024);
    printf("\nROM Speed     : %s",
           snesheader.romlayout & 0x10 ? "Fast (3.58MHz)" : "Slow (2.68MHz)");
    printf("\nBank size:    : %s", rom_is_lorom ? "LoROM" : "HiROM");
    printf("\nChipset       : %s", types[add_on_chip]);
    if ((snesheader.countrycode == 0x00) || (snesheader.countrycode == 0x01) ||
        (snesheader.countrycode == 0x0D))
      printf("\nVideo Type    : NTSC (60Hz)");
    else
      printf("\nVideo Type    : PAL (50Hz)");
    printf("\nSRAM Size     : %dK [%02X]", memory_amount[snesheader.SRAMsize],
           snesheader.SRAMsize);
    printf("\nExp.RAM Size  : %dK",
           snesheader.license == 0x33 ? snesheader.extended[13] : 0);
    printf("\nBattery       : %s", has_sram ? "Yes" : "No");
    printf("\nNMI Address   : %04Xh (native) %04Xh (emul.)",
           snesheader.nmi_vecs, snesheader.enmi_vecs);
...
int change_checksum(char *filename, FILE *fp) {
  ...
  addr = rom_is_lorom ? LOROM_HEADER + OFFSET_CHECKSUM
  ...
  for (i = 0; i < addr; ++i) {
    if ((c = fgetc(fp)) == EOF) {
      printf("\nERROR: Unexpected end of file [%s] af offset %x.", filename, i);
      return 0;
    }
  ...
  if (quietmode == 0)
    printf("\nChange Checksum to %04Xh and Checksum complement %04Xh..",
...
int change_title(char *filename, FILE *fp, char *title) {
  ...
  if (quietmode == 0) printf("\nChange title to [%s] ...", title);

  // Compute address of title entry.
  addr = 512 * rom_has_header + rom_is_lorom ? LOROM_HEADER + OFFSET_TITLE
                                             : HIROM_HEADER + OFFSET_TITLE;

  // Go to title and change it
  fseek(fp, addr, SEEK_SET);
  for (i = 0; i < 21; i++) fputc(title[i], fp);
...
int change_country(char *filename, FILE *fp, char *country) {
  ...
  addr = 512 * rom_has_header + rom_is_lorom ? LOROM_HEADER + OFFSET_COUNTRY
                                             : HIROM_HEADER + OFFSET_COUNTRY;
...
int change_sram(char *filename, FILE *fp, char nosram) {
  ...
  addr = 512 * rom_has_header + rom_is_lorom ? LOROM_HEADER + OFFSET_SRAM
                                             : HIROM_HEADER + OFFSET_SRAM;
  fseek(fp, addr, SEEK_SET);
  if (nosram == 1) fputc(0, fp);

  // Compute address of title entry.
  addr = 512 * rom_has_header + rom_is_lorom ? LOROM_HEADER + OFFSET_CARDTYPE
                                             : HIROM_HEADER + OFFSET_CARDTYPE;
  fseek(fp, addr, SEEK_SET);
  if (nosram == 1) fputc(0, fp);
...
void PrintOptions(char *str) {
  printf("\n\nUsage : snestools [options] sfc/smc filename ...");
...
int main(int argc, char **arg) {
  ...
  for (i = 1; i < argc; i++) {
    if (arg[i][0] == '-') {
      ...
      {
        quietmode = 1;
      } else if (arg[i][1] == 'h')  // header options
      {
        if (strcmp(&arg[i][1], "hi!") == 0) {
          showheader = 0;
        ...
        {
          strcpy(programtitle, &arg[i][3]);
          strtoupper(programtitle);
          if (strlen(programtitle)) {
            for (j = strlen(programtitle); j < 21; j++) programtitle[j] = ' ';
  ...
  if (filebase[0] == 0) {
    printf("\nERROR: You must specify a snes filename.");
    PrintOptions("");
    return 1;
  }
  if ((changetitle) && (strlen(programtitle) == 0)) {
    printf("\nERROR: You must specify a game title to change it.");
    PrintOptions("");
    return 1;
  }
